...
xt <- as.numeric(SZZZ[,"000001.SS.Close"])
length(xt) #896

ttt <- as.POSIXct(as.numeric(.indexDate(SZZZ))*86400, origin = "1970-01-01", format="%Y-%m-%d", tz = "UTC") 
att <- seq(as.Date("2008-1-1"), as.Date("2014-9-30"), by="year")
...
trend30 <- lm(y~poly(x, 3), data.frame(x=1:length(xt), y=kospi200d$residue))
