...
ac_model<-function(x,y,i)
{
  ...
  dfddfd<-aaply(y,1,"lsfit_x",as.numeric(x)[3:length(x)], .parallel = T)
...
ac_indicator<-function(x,y,i,j)
{
  ...
  result<-aaply(pred_data,1,"ac_model",y=sample_data,i=5, .parallel = T)
...
system.time({testdata112121<-laply(list.files(stockdata_dir),"get_pred_data",stockdata_dir,pre_date,SZZS,.parallel = T)
})
